...
const firebaseConfig = {
  ...
  databaseURL: "https://kintai-app-76bb7-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "kintai-app-76bb7",
  storageBucket: "kintai-app-76bb7.appspot.com",
...
document.addEventListener('DOMContentLoaded', () => {
  initMessaging();
  requestPermissionAndGetToken(); // ← messaging が null のときは無視されるようにしておく
});
...
// ESMならトップレベルawait可。難しければ IIFEでもOK。
await loadWebhookConfig();

const GMAIL_ONLY = false; // Gmail限定にしたいなら true
...
async function notifyAdmins(kind, payload) {
  ...
  const buildUrl = (to) => {
    const u = new URL(window.NOTIFY_WEBHOOK_URL); // /exec
  ...
  const ping = (to) => {
    ...
    img.src = url;          // ← これで確実に GET
    // GC防止に保持（任意）
    window.__lastPing = img;
    return Promise.resolve();
  };

  const tasks = admins.map(a => ping({ name: a.name, email: a.email }));
  await Promise.allSettled(tasks);

  // デバッグ：実際に投げたURLの頭を出す
  try {
    console.log('[notifyAdmins] img GET ->',
      buildUrl({ name: admins[0].name, email: admins[0].email }).slice(0,140)+'…');
  } catch {}
}
...
async function requestPermissionAndGetToken() {
  if (!messaging) {
    console.warn('[FCM] この環境ではWeb Pushがサポートされていません');
    return null;
  }
  ...
  try {
    ...
    console.log('[FCM] token saved to RTDB');
  } catch (e) {
    console.warn('[FCM] failed to save token:', e);
  }

  console.log('FCMトークン:', token); // ← これをコンソールからコピーしてテスト送信
  return token;
}
